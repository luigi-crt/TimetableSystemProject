...
public class TimetableGenerator {
            ...
            for (Module module : modules) {
                ...
                for (int i = 0; i < module.getLectureHours(); i++) {
                    ScheduledClass sc = scheduleSession(
                            module, group, "lecture", timetable, usedSlots, days, times, lecturers.get(lecturerIndex), false
                    );
                ...
                for (int i = 0; i < module.getTutorialHours(); i++) {
                    ScheduledClass sc = scheduleSession(
                            module, group, "tutorial", timetable, usedSlots, days, times, lecturers.get(lecturerIndex), false
                    );
                    if (sc != null) timetable.add(sc);
                    lecturerIndex = (lecturerIndex + 1) % lecturers.size();
                }
                ...
                for (int i = 0; i < module.getLabHours(); i++) {
                    ScheduledClass sc = scheduleSession(
                            module, group, "lab", timetable, usedSlots, days, times, lecturers.get(lecturerIndex), true
                    );
                    if (sc != null) timetable.add(sc);
                    lecturerIndex = (lecturerIndex + 1) % lecturers.size();
                }
    ...
    // Helper method to schedule a session of a given type
